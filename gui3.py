...
def open_image():
    ...
    if file_path:
        ...
        # Classify the image
        sign, confidence = classify(image)
        ...
        sign_image.config(image=img)
        sign_image.image = img

        label.config(text=f"Biển báo: {sign}\nĐộ chính xác: {confidence:.2f}%")
        label.pack()
...
def open_camera():
    ...
    # process_video("http://9.212.12.13:8080/video")  # 0 is the default camera
...
# Buttons for selecting image, video, and camera
image_button = Button(top, text="Chọn ảnh", command=open_image, padx=10, pady=5)
image_button.configure(background='#364156', foreground='white', font=('arial', 10, 'bold'))
image_button.pack(side=TOP, pady=5)

video_button = Button(top, text="Mở Video", command=open_video_file, padx=10, pady=5)
video_button.configure(background='#364156', foreground='white', font=('arial', 10, 'bold'))
video_button.pack(side=TOP, pady=5)

camera_button = Button(top, text="Mở Camera", command=open_camera, padx=10, pady=5)
camera_button.configure(background='#364156', foreground='white', font=('arial', 10, 'bold'))
camera_button.pack(side=TOP, pady=5)
